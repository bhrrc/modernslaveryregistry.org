class CompanySearchService
  ...
  def search_by_conditions
    ...
    Company.search({body: { query: { bool: conditions }, sort: { name: 'asc' }, track_total_hits: true, select: [] }}.merge(limit_options))
  end

  def search_all
    Company.search({body: { query: { match_all: {} }, sort: { name: 'asc' }, track_total_hits: true, select: [] }}.merge(limit_options))
  end
