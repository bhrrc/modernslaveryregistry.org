...
RSpec.describe Industry, type: :model do
  ...
  it 'reports number of industries with companies' do
    Company.create!(industry_id: @agriculture.id, name: 'Cucumber Ltd', country_id: @gb.id)
    expect(Industry.with_companies.all.length).to eq(1)
  end

  it 'reports countries in each industry' do
    Company.create!(industry_id: @agriculture.id, name: 'Ag1', country_id: @gb.id)
    Company.create!(industry_id: @agriculture.id, name: 'Ag2', country_id: @fr.id)
    Company.create!(industry_id: @agriculture.id, name: 'Ag3', country_id: @gb.id)
    Company.create!(industry_id: @software.id, name: 'Sw1', country_id: @gb.id)
